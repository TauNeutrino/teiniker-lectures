...
While a stateless protocol works fine for many simple static content 
sites, most modern Web applications need to keep track of what users 
are currently connected to them (a.k.a. session) and the data associated
with those sessions (a.k.a. state).

The most convenient way to keep track of a session is for a server to 
issue each client a session ID - a unique number that identifies that 
client.
With each request the client sends back the session ID, which the server 
can then use to associate with existing state information for that 
particular client.
This session ID also has the affect of extending authentication, since 
once a client authenticates, the server can associate the session ID 
with that particular authenticated entity.
